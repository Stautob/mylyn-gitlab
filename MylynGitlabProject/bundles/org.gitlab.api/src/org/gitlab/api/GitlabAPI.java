...
public class GitlabAPI {
   ...
   public GitlabAPI setResponseReadTimeout(int readTimeout) {
      if (readTimeout < 0) {
         LOG.warn("The value of the \"Response Read Timeout\" parameter can not be negative. " + "The default value [{}] will be used.",
               defaultTimeout);
   ...
   public GitlabAPI setConnectionTimeout(int connectionTimeout) {
      if (connectionTimeout < 0) {
         LOG.warn("The value of the \"Connection Timeout\" parameter can not be negative. " + "The default value [{}] will be used.", defaultTimeout);
    ...
    *        Some portion of the email address or username
    ...
    */
   public GitlabUser createUser(String email, String password, String username, String fullName, String skypeId, String linkedIn, String twitter,
         String website_url, Integer projects_limit, String extern_uid, String extern_provider_name, String bio, Boolean isAdmin,
         Boolean can_create_group, Boolean skip_confirmation, Boolean external) throws IOException {

      Query query = new Query().append("email", email).appendIf("skip_confirmation", skip_confirmation).appendIf("password", password).appendIf(
            "username", username).appendIf("name", fullName).appendIf("skype", skypeId).appendIf("linkedin", linkedIn).appendIf("twitter", twitter)
            .appendIf("website_url", website_url).appendIf("projects_limit", projects_limit).appendIf("extern_uid", extern_uid).appendIf("provider",
                  extern_provider_name).appendIf("bio", bio).appendIf("admin", isAdmin).appendIf("can_create_group", can_create_group).appendIf(
                        "external", external);
    ...
    *
    * @param request
    *        An object that represents the parameters for the request.
    ...
    *         on gitlab api call error
    */
   public GitlabUser updateUser(Integer targetUserId, String email, String password, String username, String fullName, String skypeId,
         String linkedIn, String twitter, String website_url, Integer projects_limit, String extern_uid, String extern_provider_name, String bio,
         Boolean isAdmin, Boolean can_create_group, Boolean external) throws IOException {

      Query query = new Query().append("email", email).appendIf("password", password).appendIf("username", username).appendIf("name", fullName)
            .appendIf("skype", skypeId).appendIf("linkedin", linkedIn).appendIf("twitter", twitter).appendIf("website_url", website_url).appendIf(
                  "projects_limit", projects_limit).appendIf("extern_uid", extern_uid).appendIf("provider", extern_provider_name).appendIf("bio", bio)
            .appendIf("admin", isAdmin).appendIf("can_create_group", can_create_group).appendIf("external", external);
   ...
   public List<GitlabGroup> getGroupsViaSudo(String username, Pagination pagination) throws IOException {
      ...
      Query query = new Query().appendIf(PARAM_SUDO, username);
    ...
    *        The path for the group
    * @param sudoUser
    *        The user to create the group on behalf of
    * @return The GitLab Group
    * @throws IOException
    *         on gitlab api call error
    */
    ...
    *        An object that represents the parameters for the request.
    * @param sudoUser
    *        The user for whom we're creating the group
    * @return The GitLab Group
    * @throws IOException
    *         on gitlab api call error
    */
    ...
    *        The user to create the group on behalf of
    * @param parentId
    *        The id of a parent group; the new group will be its subgroup
    * @return The GitLab Group
    * @throws IOException
    *         on gitlab api call error
    */
   public GitlabGroup createGroup(String name, String path, String ldapCn, GitlabAccessLevel ldapAccess, GitlabUser sudoUser, Integer parentId)
         throws IOException {

      Query query = new Query().append("name", name).append("path", path).appendIf("ldap_cn", ldapCn).appendIf("ldap_access", ldapAccess).appendIf(
            PARAM_SUDO, sudoUser != null ? sudoUser.getId() : null).appendIf("parent_id", parentId);
   ...
   public GitlabGroup createGroup(GitlabGroup group, GitlabUser sudoUser) throws IOException {
      ...
      Query query = new Query().append("name", group.getName()).append("path", group.getPath()).appendIf("description", group.getDescription())
            .appendIf("membership_lock", group.getMembershipLock()).appendIf("share_with_group_lock", group.getShareWithGroupLock()).appendIf(
                  "visibility", group.getVisibility().toString()).appendIf("lfs_enabled", group.isLfsEnabled()).appendIf("request_access_enabled",
                        group.isRequestAccessEnabled()).appendIf("shared_runners_minutes_limit", group.getSharedRunnersMinutesLimit()).appendIf(
                              "ldap_cn", group.getLdapCn()).appendIf("ldap_access", group.getLdapAccess()).appendIf(PARAM_SUDO, sudoUser != null
                                                                                                                                                 ? sudoUser
                                                                                                                                                       .getId()
                                                                                                                                                 : null);
   ...
   public GitlabGroup updateGroup(GitlabGroup group, GitlabUser sudoUser) throws IOException {
      ...
      Query query = new Query().appendIf("name", group.getName()).appendIf("path", group.getPath()).appendIf("description", group.getDescription())
            .appendIf("membership_lock", group.getMembershipLock()).appendIf("share_with_group_lock", group.getShareWithGroupLock()).appendIf(
                  "visibility", group.getVisibility().toString()).appendIf("lfs_enabled", group.isLfsEnabled()).appendIf("request_access_enabled",
                        group.isRequestAccessEnabled()).appendIf("shared_runners_minutes_limit", group.getSharedRunnersMinutesLimit()).appendIf(
                              "ldap_cn", group.getLdapCn()).appendIf("ldap_access", group.getLdapAccess()).appendIf(PARAM_SUDO, sudoUser != null
    ...
    *        page offset.
    * @param perPage
    *        number elements to get after page offset.
    * @return A list of gitlab projects
    * @throws IOException
    *         on Gitlab API call error
    */
   public List<GitlabProject> getProjectsWithPagination(int page, int perPage) throws IOException {
      Pagination pagination = new Pagination().withPage(page).withPerPage(perPage);
    ...
    *
    * @param user
    *        Gitlab User to invoke sudo with
    * @param page
    *        Page offset
    * @param perPage
    *        Number of elements to get after page offset
    * @return A list of gitlab projects
    * @throws IOException
    *         Gitlab API call error
    */
   ...
   public List<GitlabJob> getPipelineJobs(Integer projectId, Integer pipelineId) {
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + GitlabPipeline.URL + "/" + sanitizeId(pipelineId, "PipelineID") +
                       GitlabJob.URL + PARAM_MAX_ITEMS_PER_PAGE;
      return retrieve().getAll(tailUrl, GitlabJob[].class);
   }
   ...
   public GitlabProject createProject(GitlabProject project) throws IOException {
      Query query = new Query().appendIf("name", project.getName()).appendIf("path", project.getPath()).appendIf("default_branch", project
            .getDefaultBranch()).appendIf("description", project.getDescription()).appendIf("issues_enabled", project.isIssuesEnabled()).appendIf(
                  "merge_requests_enabled", project.isMergeRequestsEnabled()).appendIf("jobs_enabled", project.isJobsEnabled()).appendIf(
                        "wiki_enabled", project.isWikiEnabled()).appendIf("snippets_enabled", project.isSnippetsEnabled())

            .appendIf("container_registry_enabled", project.isContainerRegistryEnabled()).appendIf("shared_runners_enabled", project
                  .isSharedRunnersEnabled())

            .appendIf("visibility", project.getVisibility()).appendIf("public_jobs", project.hasPublicJobs()).appendIf("import_url", project
                  .getImportUrl())

            .appendIf("only_allow_merge_if_pipeline_succeeds", project.getOnlyAllowMergeIfPipelineSucceeds()).appendIf(
                  "only_allow_merge_if_all_discussions_are_resolved", project.getOnlyAllowMergeIfAllDiscussionsAreResolved()).appendIf("lfs_enabled",
                        project.isLfsEnabled()).appendIf("request_access_enabled", project.isRequestAccessEnabled()).appendIf("repository_storage",
                              project.getRepositoryStorage()).appendIf("approvals_before_merge", project.getApprovalsBeforeMerge()).appendIf(
                                    "printing_merge_request_link_enabled", project.isPrintingMergeRequestLinkEnabled());
    ...
    *        The name of the project
    * @param group
    *        The group for which the project should be crated
    * @return The GitLab Project
    * @throws IOException
    *         on gitlab api call error
    */
    ...
    *        The project description
    * @param visibility
    *        The project visibility level (private: 0, internal: 10, public: 20)
    * @return The GitLab Project
    * @throws IOException
    *         on gitlab api call error
    */
    ...
    *        Whether Merge Requests should be enabled, otherwise null indicates to use GitLab default
    * @param wikiEnabled
   ...
   public GitlabProject createProject(String name, Integer namespaceId, String description, Boolean issuesEnabled, Boolean wallEnabled,
         Boolean mergeRequestsEnabled, Boolean wikiEnabled, Boolean snippetsEnabled, Boolean publik, String visibility, String importUrl)
         throws IOException {
      Query query = new Query().append("name", name).appendIf("namespace_id", namespaceId).appendIf("description", description).appendIf(
            "issues_enabled", issuesEnabled).appendIf("wall_enabled", wallEnabled).appendIf("merge_requests_enabled", mergeRequestsEnabled).appendIf(
                  "wiki_enabled", wikiEnabled).appendIf("snippets_enabled", snippetsEnabled).appendIf("visibility", visibility).appendIf("import_url",
                        importUrl);
   ...
   public GitlabProject createUserProject(Integer userId, String name, String description, String defaultBranch, Boolean issuesEnabled,
         Boolean wallEnabled, Boolean mergeRequestsEnabled, Boolean wikiEnabled, Boolean snippetsEnabled, String visibility, String importUrl)
         throws IOException {
      Query query = new Query().append("name", name).appendIf("description", description).appendIf("default_branch", defaultBranch).appendIf(
            "issues_enabled", issuesEnabled).appendIf("wall_enabled", wallEnabled).appendIf("merge_requests_enabled", mergeRequestsEnabled).appendIf(
                  "wiki_enabled", wikiEnabled).appendIf("snippets_enabled", snippetsEnabled).appendIf("visibility", visibility).appendIf("import_url",
                        importUrl);
    ...
    *        A description for the project, null otherwise
    * @param defaultBranch
    *        The branch displayed in the Gitlab UI when a user navigates to the project
    * @param issuesEnabled
    *        Whether Issues should be enabled, otherwise null indicates to use GitLab default
    * @param wallEnabled
    *        Whether The Wall should be enabled, otherwise null indicates to use GitLab default
    ...
    */
   @Deprecated
   public GitlabProject updateProject(Integer projectId, String name, String description, String defaultBranch, Boolean issuesEnabled,
         Boolean wallEnabled, Boolean mergeRequestsEnabled, Boolean wikiEnabled, Boolean snippetsEnabled, String visibility) throws IOException {
      Query query = new Query().appendIf("name", name).appendIf("description", description).appendIf("default_branch", defaultBranch).appendIf(
            "issues_enabled", issuesEnabled).appendIf("wall_enabled", wallEnabled).appendIf("merge_requests_enabled", mergeRequestsEnabled).appendIf(
                  "wiki_enabled", wikiEnabled).appendIf("snippets_enabled", snippetsEnabled).appendIf("visibility", visibility);
   ...
   public GitlabMergeRequestApprovals getMergeRequestApprovals(GitlabMergeRequest mr) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(mr.getProjectId()) + GitlabMergeRequest.URL + "/" + mr.getIid() +
                       GitlabMergeRequestApprovals.URL;
    ...
    *        The branch on which the commit must be cherry-picked
    ...
    *         on gitlab api call error
    */
   public GitlabMergeRequest createMergeRequest(Serializable projectId, String sourceBranch, String targetBranch, Integer assigneeId, String title)
         throws IOException {

      Query query = new Query().appendIf("target_branch", targetBranch).appendIf("source_branch", sourceBranch).appendIf("assignee_id", assigneeId)
    ...
    * @param targetBranch
    *        The target branch of the merge request, otherwise null to leave it untouched
    * @param assigneeId
    *        The id of the assignee, otherwise null to leave it untouched
    * @param title
    *        The title of the merge request, otherwise null to leave it untouched
    * @param description
    *        The description of the merge request, otherwise null to leave it untouched
    * @param stateEvent
    *        The state (close|reopen|merge) of the merge request, otherwise null to leave it untouched
    * @param labels
    *        A comma separated list of labels, otherwise null to leave it untouched
    * @return the Merge Request
    * @throws IOException
    *         on gitlab api call error
    */
   public GitlabMergeRequest updateMergeRequest(Serializable projectId, Integer mergeRequestId, String targetBranch, Integer assigneeId, String title,
         String description, String stateEvent, String labels) throws IOException {
      Query query = new Query().appendIf("target_branch", targetBranch).appendIf("assignee_id", assigneeId).appendIf("title", title).appendIf(
            "description", description).appendIf("state_event", stateEvent).appendIf("labels", labels);
    ...
    *        optional merge commit message. Null if not set
   ...
   public GitlabMergeRequest acceptMergeRequest(Serializable projectId, Integer mergeRequestId, String mergeCommitMessage) throws IOException {
      ...
      if (mergeCommitMessage != null) requestor.with("merge_commit_message", mergeCommitMessage);
   ...
   public GitlabNote getNote(GitlabMergeRequest mergeRequest, Integer noteId) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + mergeRequest.getProjectId() + GitlabMergeRequest.URL + "/" + mergeRequest.getIid() + GitlabNote.URL +
                       "/" + noteId;
   ...
   public List<GitlabCommit> getCommits(GitlabMergeRequest mergeRequest, Pagination pagination) throws IOException {
      ...
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + "/repository" + GitlabCommit.URL + query.toString();
   ...
   // List commit diffs for a project ID and two commit hashes
   // GET /projects/:id/repository/compare
   public GitlabCommitComparison compareCommits(Serializable projectId, String commitHash1, String commitHash2, Pagination pagination)
         throws IOException {
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + GitlabCommitComparison.URL;
      Query query = new Query().append("from", commitHash1).append("to", commitHash2);
      query.mergeWith(pagination.asQuery());
   ...
   public List<GitlabCommitStatus> getCommitStatuses(Serializable projectId, String commitHash, Pagination pagination) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + "/repository" + GitlabCommit.URL + "/" + commitHash +
                       GitlabCommitStatus.URL + pagination;
   ...
   public GitlabCommitStatus createCommitStatus(GitlabProject project, String commitHash, String state, String ref, String name, String targetUrl,
         String description) throws IOException {
      return createCommitStatus(project.getId(), commitHash, state, ref, name, targetUrl, description);
   }

   public GitlabCommitStatus createCommitStatus(Serializable projectId, String commitHash, String state, String ref, String name, String targetUrl,
         String description) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + GitlabCommitStatus.URL + "/" + commitHash;
      return dispatch().with("state", state).with("ref", ref).with("name", name).with("target_url", targetUrl).with("description", description).to(
            tailUrl, GitlabCommitStatus.class);
   }
   ...
   public byte[] getRawFileContent(Serializable projectId, String sha, String filepath) throws IOException {
      ...
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + "/repository/files/" + sanitizePath(filepath) + "/raw" + query
            .toString();
      return retrieve().to(tailUrl, byte[].class);
   }
    ...
    *        The Project
    * @param path
    *        The path inside the repository. Used to get content of subdirectories (optional)
    * @param ref
    *        The name of a repository branch or tag or if not given the default branch (optional)
    * @throws IOException
    *         on gitlab api call error
    */
   public List<GitlabRepositoryTree> getRepositoryTree(GitlabProject project, String path, String ref, boolean recursive) throws IOException {
      Query query = new Pagination().withPerPage(Pagination.MAX_ITEMS_PER_PAGE).asQuery().appendIf("path", path).appendIf("ref", ref).appendIf(
            "recursive", recursive);
   ...
   public GitlabRepositoryFile getRepositoryFile(GitlabProject project, String path, String ref) throws IOException {
      Query query = new Query().append("ref", ref);
    ...
    * @param branchName
    *        The name of a repository branch
    * @param commitMsg
    *        The commit message
    * @param content
    ...
    *         on gitlab api call error
    */
   public GitlabSimpleRepositoryFile createRepositoryFile(GitlabProject project, String path, String branchName, String commitMsg, String content)
         throws IOException {
      String tailUrl = GitlabProject.URL + "/" + project.getId() + "/repository/files/" + sanitizePath(path);
      GitlabHTTPRequestor requestor = dispatch();

      return requestor.with("branch", branchName).with("encoding", "base64").with("commit_message", commitMsg).with("content", content).to(tailUrl,
            GitlabSimpleRepositoryFile.class);
   }
    ...
    *         on gitlab api call error
    */
   public GitlabSimpleRepositoryFile updateRepositoryFile(GitlabProject project, String path, String branchName, String commitMsg, String content)
         throws IOException {
      String tailUrl = GitlabProject.URL + "/" + project.getId() + "/repository/files/" + sanitizePath(path);
      GitlabHTTPRequestor requestor = retrieve().method(PUT);
   ...
   public void deleteRepositoryFile(GitlabProject project, String path, String branchName, String commitMsg) throws IOException {
      Query query = new Query().append("branch", branchName).append("commit_message", commitMsg);
      String tailUrl = GitlabProject.URL + "/" + project.getId() + "/repository/files/" + sanitizePath(path) + query.toString();
      retrieve().method(DELETE).to(tailUrl, Void.class);
   }
   ...
   public void protectBranchWithDeveloperOptions(GitlabProject project, String branchName, boolean developers_can_push, boolean developers_can_merge)
         throws IOException {
      String tailUrl = GitlabProject.URL + "/" + project.getId() + GitlabBranch.URL + '/' + sanitizePath(branchName) + "/protect";
      final Query query = new Query().append("developers_can_push", Boolean.toString(developers_can_push)).append("developers_can_merge", Boolean
            .toString(developers_can_merge));
   ...
   public GitlabProjectHook addProjectHook(GitlabProject project, String url, String token) throws IOException {
      ...
      return dispatch().with("url", url).with("token", token).to(tailUrl, GitlabProjectHook.class);
   }

   public GitlabProjectHook addProjectHook(Serializable projectId, String url, boolean pushEvents, boolean issuesEvents, boolean mergeRequestEvents,
         boolean noteEvents, boolean tagPushEvents, boolean sslVerification, String token) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + GitlabProjectHook.URL;

      return dispatch().with("url", url).with("push_events", pushEvents ? "true" : "false").with("issues_events", issuesEvents ? "true" : "false")
            .with("merge_requests_events", mergeRequestEvents ? "true" : "false").with("note_events", noteEvents ? "true" : "false").with(
                  "tag_push_events", tagPushEvents ? "true" : "false").with("enable_ssl_verification", sslVerification ? "true" : "false").with(
                        "token", token).to(tailUrl, GitlabProjectHook.class);
   }
   ...
   public List<GitlabIssue> getIssues(GitlabProject project, GitlabMilestone milestone) {
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(project.getId()) + GitlabMilestone.URL + "/" + sanitizeMilestoneId(milestone
            .getId()) + GitlabIssue.URL + PARAM_MAX_ITEMS_PER_PAGE;
      return retrieve().getAll(tailUrl, GitlabIssue[].class);
   }

   public List<GitlabIssue> getIssues(GitlabGroup group, GitlabMilestone milestone) {
      String tailUrl = GitlabGroup.URL + "/" + sanitizeGroupId(group.getId()) + GitlabMilestone.URL + "/" + sanitizeMilestoneId(milestone.getId()) +
   ...
   public GitlabIssue createIssue(int projectId, int assigneeId, Integer milestoneId, String labels, String description, String title)
         throws IOException {
   ...
   private void applyIssue(GitlabHTTPRequestor requestor, int projectId, int assigneeId, Integer milestoneId, String labels, String description,
         String title) {
   ...
   public GitlabNote getNote(GitlabIssue issue, Integer noteId) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + issue.getProjectId() + GitlabIssue.URL + "/" + issue.getIid() + GitlabNote.URL + "/" + noteId;
      return retrieve().to(tailUrl, GitlabNote.class);
   }

   public List<GitlabNote> getNotes(GitlabIssue issue) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + issue.getProjectId() + GitlabIssue.URL + "/" + issue.getIid() + GitlabNote.URL;
   ...
   public GitlabNote createNote(Serializable projectId, Integer issueId, String message) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + GitlabIssue.URL + "/" + issueId + GitlabNote.URL;
   ...
   public void deleteNote(Serializable projectId, Integer issueId, GitlabNote noteToDelete) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + GitlabIssue.URL + "/" + issueId + GitlabNote.URL + "/" + noteToDelete
            .getId();
      retrieve().method(DELETE).to(tailUrl, GitlabNote.class);
   }
    ...
    *
    * @param projectId
    *        The id of the project for which the badges should be retrieved
    ...
    *
    * @param projectId
    *        The id of the project for which the badge should be retrieved
    * @param badgeId
    *        The id of the badge that should be retrieved
   ...
   public GitlabBadge getProjectBadge(Serializable projectId, Integer badgeId) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + GitlabBadge.URL + "/" + badgeId;
    ...
    *
    * @param projectId
    *        The id of the project for which the badge should be added
    * @param linkUrl
    *        The URL that the badge should link to
    * @param imageUrl
   ...
   public GitlabBadge addProjectBadge(Serializable projectId, String linkUrl, String imageUrl) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + GitlabBadge.URL;
      return dispatch().with("link_url", linkUrl).with("image_url", imageUrl).to(tailUrl, GitlabBadge.class);
   }
    ...
    *
    * @param projectId
    *        The id of the project for which the badge should be edited
    * @param badgeId
    *        The id of the badge that should be edited
    * @param linkUrl
    *        The URL that the badge should link to
    * @param imageUrl
    *        The URL to the badge image
   ...
   public GitlabBadge editProjectBadge(Serializable projectId, Integer badgeId, String linkUrl, String imageUrl) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + GitlabBadge.URL + "/" + badgeId;
      GitlabHTTPRequestor requestor = retrieve().method(PUT);
      requestor.with("link_url", linkUrl).with("image_url", imageUrl);
    ...
    *
    * @param projectId
    *        The id of the project for which the badge should be deleted
    * @param badgeId
    *        The id of the badge that should be deleted
    * @throws IOException
    *         on GitLab API call error
    */
    ...
    *
    * @param groupId
    *        The id of the group for which the badges should be retrieved
    * @return The list of badges
    *
    * @throws IOException
    *         on GitLab API call error
    ...
    *
    * @param groupId
    *        The id of the group for which the badge should be retrieved
    * @param badgeId
    *        The id of the badge that should be retrieved
    * @return The badge with a given id
    *
   ...
   public GitlabBadge getGroupBadge(Integer groupId, Integer badgeId) throws IOException {
      String tailUrl = GitlabGroup.URL + "/" + groupId + GitlabBadge.URL + "/" + badgeId;
      return retrieve().to(tailUrl, GitlabBadge.class);
   }
    ...
    *
    * @param groupId
    *        The id of the group for which the badge should be added
    * @param linkUrl
    *        The URL that the badge should link to
    * @param imageUrl
    *        The URL to the badge image
    ...
    *
    * @param groupId
    *        The id of the group for which the badge should be edited
    * @param badgeId
    *        The id of the badge that should be edited
    * @param linkUrl
    *        The URL that the badge should link to
    ...
    *
    * @param groupId
    *        The id of the group for which the badge should be deleted
    * @param badgeId
    *        The id of the badge that should be deleted
    * @throws IOException
    *         on GitLab API call error
    ...
    *
    * @param project
    *        The project associated with labels.
    * @return A non-null list of labels.
    * @throws IOException
    *         on gitlab api call error
    */
    ...
    *
    * @param projectId
    *        The ID of the project containing the new label.
    * @param name
    *        The name of the label.
    * @param color
    *        The color of the label (eg #ff0000).
   ...
   public GitlabLabel createLabel(Serializable projectId, String name, String color) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + GitlabLabel.URL;
      return dispatch().with("name", name).with("color", color).to(tailUrl, GitlabLabel.class);
   }
    ...
    *
    * @param projectId
    *        The ID of the project containing the label.
    * @param label
    *        The label to create.
    * @return The newly created label.
    */
   ...
   public void deleteLabel(Serializable projectId, String name) throws IOException {
      ...
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + GitlabLabel.URL + query.toString();
      retrieve().method(DELETE).to(tailUrl, Void.class);
   }
    ...
    * @param description
    *        The description of the milestone. (Optional)
    * @param dueDate
    *        The date the milestone is due. (Optional)
    * @param startDate
    *        The start date of the milestone. (Optional)
    ...
    * @param title
    *        The title of the milestone. (Optional)
    * @param description
    *        The description of the milestone. (Optional)
    * @param dueDate
    *        The date the milestone is due. (Optional)
    ...
    *         on gitlab api call error
    */
   public GitlabMilestone updateMilestone(Serializable projectId, int milestoneId, String title, String description, Date dueDate, Date startDate,
         String stateEvent) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + GitlabMilestone.URL + "/" + milestoneId;
      GitlabHTTPRequestor requestor = retrieve().method(PUT);
      SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
      if (title != null) {
   ...
   public GitlabMilestone updateMilestone(Serializable projectId, GitlabMilestone edited, String stateEvent) throws IOException {
      return updateMilestone(projectId, edited.getId(), edited.getTitle(), edited.getDescription(), edited.getDueDate(), edited.getStartDate(),
            stateEvent);
   }
    ...
    *        the user id
    * @param accessLevel
    *        the updated access level for the specified user
    ...
    *        the updated access level for the specified user
    * @param expiresAt
    *        the date at which the user's membership expires at in the form YEAR-MONTH-DAY
    * @return GitLabProjectMember with updated access level on success
    * @throws IOException
    *         on Gitlab API call error
    */
   public GitlabProjectMember updateProjectMember(Integer projectId, Integer userId, GitlabAccessLevel accessLevel, String expiresAt)
         throws IOException {
      Query query = new Query().appendIf("access_level", accessLevel).appendIf("expires_at", expiresAt);
   ...
   private GitlabSSHKey createDeployKey(Integer targetProjectId, String title, String key, boolean canPush) throws IOException {
      Query query = new Query().append("title", title).append("key", key).append("can_push", Boolean.toString(canPush));
   ...
   private String sanitizeId(Serializable id, String parameterName) {
      if (!(id instanceof String) && !(id instanceof Number)) { throw new IllegalArgumentException(parameterName +
                                                                                                   " needs to be of type String or Number"); }
    ...
    * @param line_type
    *        (optional) - The line type (new or old)
    * @return A CommitComment
    * @throws IOException
    *         on gitlab api call error
    * @see <a href=
    *      "http://doc.gitlab.com/ce/api/commits.html#post-comment-to-commit">http://doc.gitlab.com/ce/api/commits.html#post-comment-to-commit</a>
    */
   public CommitComment createCommitComment(Serializable projectId, String sha, String note, String path, String line, String line_type)
         throws IOException {

      Query query = new Query().append("id", projectId.toString()).appendIf("sha", sha).appendIf("note", note).appendIf("path", path).appendIf("line",
            line).appendIf("line_type", line_type);
   ...
   public GitlabTag addTag(Serializable projectId, String tagName, String ref, String message, String releaseDescription) throws IOException {
      ...
      return dispatch().with("tag_name", tagName).with("ref", ref).with("message", message).with("release_description", releaseDescription).to(
            tailUrl, GitlabTag.class);
   }
   ...
   public List<GitlabAward> getAllAwards(GitlabMergeRequest mergeRequest) {
      String tailUrl = GitlabProject.URL + "/" + mergeRequest.getProjectId() + GitlabMergeRequest.URL + "/" + mergeRequest.getIid() +
   ...
   public List<GitlabAward> getAllAwards(GitlabIssue issue) {
      String tailUrl = GitlabProject.URL + "/" + issue.getProjectId() + GitlabIssue.URL + "/" + issue.getId() + GitlabAward.URL +
                       PARAM_MAX_ITEMS_PER_PAGE;
   ...
   public GitlabAward getAward(GitlabIssue issue, Integer awardId) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + issue.getProjectId() + GitlabIssue.URL + "/" + issue.getId() + GitlabAward.URL + "/" + awardId;
   ...
   public GitlabAward createAward(GitlabIssue issue, String awardName) throws IOException {
      Query query = new Query().append("name", awardName);
      String tailUrl = GitlabProject.URL + "/" + issue.getProjectId() + GitlabIssue.URL + "/" + issue.getId() + GitlabAward.URL + query.toString();
   ...
   public void deleteAward(GitlabIssue issue, GitlabAward award) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + issue.getProjectId() + GitlabIssue.URL + "/" + issue.getId() + GitlabAward.URL + "/" + award.getId();
      retrieve().method(DELETE).to(tailUrl, Void.class);
   }
   ...
   public List<GitlabAward> getAllAwards(GitlabIssue issue, Integer noteId) {
      String tailUrl = GitlabProject.URL + "/" + issue.getProjectId() + GitlabIssue.URL + "/" + issue.getId() + GitlabNote.URL + noteId +
                       GitlabAward.URL + PARAM_MAX_ITEMS_PER_PAGE;
    ...
    *         on gitlab api call error
    */
   public List<GitlabBuildVariable> getBuildVariables(Integer projectId) throws IOException {
    ...
    *
    * @param project
    *        The project associated with variables.
    * @return A non-null list of variables.
    * @throws IOException
    *         on gitlab api call error
    */
   ...
   public GitlabBuildVariable getBuildVariable(Integer projectId, String key) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + projectId + GitlabBuildVariable.URL + "/" + key;
    ...
    *
    * @param project
    *        The project associated with the variable.
    * @return A variable.
    * @throws IOException
    *         on gitlab api call error
    */
    ...
    *
    * @param projectId
    *        The ID of the project containing the new variable.
    * @param key
    *        The key of the variable.
    * @param value
   ...
   public GitlabBuildVariable createBuildVariable(Integer projectId, String key, String value) throws IOException {
      String tailUrl = GitlabProject.URL + "/" + projectId + GitlabBuildVariable.URL;
      return dispatch().with("key", key).with("value", value).to(tailUrl, GitlabBuildVariable.class);
   }
    ...
    *
    * @param projectId
    *        The ID of the project containing the variable.
    * @param variable
    ...
    *         if jobs are not enabled for the project
    */
    ...
    *        The ID of the project containing the variable.
    * @param emailAddress
    *        The emailaddress of the recipent who is going to receive push notification.
    * @return
    * @throws IOException
    *         on gitlab api call error
    */
   public boolean updateEmailsOnPush(Integer projectId, String emailAddress) throws IOException {
      ...
      if (appendedRecipients != "") {
         if (appendedRecipients.contains(emailAddress)) return true;
         appendedRecipients = appendedRecipients + " " + emailAddress;
      } else appendedRecipients = emailAddress;

      Query query = new Query().appendIf("active", true).appendIf("recipients", appendedRecipients);
   ...
   public boolean createOrEditJiraService(Integer projectId, GitlabJiraProperties jiraPropties) throws IOException {
      ...
      Query query = new Query().appendIf("url", jiraPropties.getUrl()).appendIf("project_key", jiraPropties.getProjectKey());
   ...
   public List<GitlabProject> searchProjects(String search) throws IOException {
      Query query = new Query().append("search", search);
      String tailUrl = GitlabProject.URL + query.toString();
    ...
    *
    * @param accessLevel
    *        The permissions level to grant the group.
    * @param group
    *        The group to share with.
    * @param project
    *        The project to be shared.
    * @param expiration
    *        Share expiration date in ISO 8601 format: 2016-09-26 or {@code null}.
    * @throws IOException
    *         on gitlab api call error
    */
   public void shareProjectWithGroup(GitlabAccessLevel accessLevel, String expiration, GitlabGroup group, GitlabProject project) throws IOException {
      Query query = new Query().append("group_id", group.getId().toString()).append("group_access", String.valueOf(accessLevel.accessValue)).appendIf(
    ...
    *
    * @param scope
    *        Can be null. Defines type of Runner to retrieve.
    * @return List of GitLabRunners
    * @throws IOException
    *         on Gitlab API call error
    */
    ...
    *        Can be null. Defines type of Runner to retrieve.
    * @param page
    *        Page to get perPage number of Runners from.
    * @param perPage
    ...
    *        Can be null. Defines type of Runner to retrieve.
    * @param pagination
    *        Can be null. Pagination to query by.
    * @return List of GitlabRunners
    * @throws IOException
    *         on Gitlab API call error
    */
   public List<GitlabRunner> getRunnersWithPagination(GitlabRunner.RunnerScope scope, Pagination pagination) throws IOException {
      ...
      Query query = new Query().appendIf("scope", scope.getScope());
